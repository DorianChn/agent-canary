...
许可机制：赞助网关签发 **绑定设备的 30 天 Ed25519 签名许可**（设备指纹 = 主机名/系统/架构/MAC 的 sha256，每个订阅最多 3 台机器）。激活时联网校验一次订阅，本地缓存签名许可 + 离线宽限；到期后重新跑同一条命令即可。许可文件每次加载都验签——手改 `expiresAt` 会让签名失效，系统时钟回拨也会被水位线检测识破。
